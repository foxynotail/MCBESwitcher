Func _launchPreview($selected_profile, $current_profile, $selected_version, $current_version)

   Local $function_name = "Play Preview"
   ...
   If $selected_profile <> $current_profile OR $selected_version <> $current_version THEN
      Local $confirm = MsgBox(4, "Launching Minecraft Preview", "Are you sure you want to launch version " & $selected_version & " with the " & $selected_profile & " profile?")
      If $confirm > 6 Then
         Return True
      EndIf
   EndIf
   ...
   $result = _checkForeignInstall($MC_Preveiw_AppX_Name, $selected_version)
   ...
   If $external_install = True Then
      MsgBox(0, "Error", "There is another version of Minecraft Preview installed on this system!" & @CRLF & @CRLF & "You need to uninstall that version first before you can use this app." & @CRLF & @CRLF & "To uninstall, please use the UNINSTALL PREVIEW button.")
      return True
   EndIf
   ...
   If($version_change==True) Then
      ...
      If _switchVersion('preview', $selected_version) == False Then
      ...
      IniWrite($options_file, "Data", "current_preview_version", $selected_version)
      EndIf
   EndIf
   ...
   _log("Switch Profile Confirmed", $function_name)

   If _switchProfile('preview', $selected_profile, $current_profile) == False Then
   ...
   _log("Launching Minecraft Preview", $function_name)
   Local $powershell_command = "explorer.exe shell:AppsFolder\" & $MC_Preview_AppX_FamilyName & "!App"
